...
public class pz390 {
	...
	private void init_opcode_keys() {
		...
		if (tz390.op_code.length != tz390.op_trace_type.length){                // RPI 1209
			tz390.abort_error(22,"op code and op trace type tables out of sync "
				+ tz390.op_code.length + " vs " + tz390.op_trace_type.length);  // RPI 1209
		}
		String hex_key = null;
		int index = 1;
	...
	private void trace_ins(){
		...
		int    maxlen = 0;
		if (tz390.op_code_index > 0
			&& tz390.op_code_index < tz390.op_trace_type.length){   // RPI 1209
			trace_type = tz390.op_trace_type[tz390.op_code_index];  // RPI 1209
		}
	try {
		switch (trace_type){
		case 10: // "E" 8 PR oooo
